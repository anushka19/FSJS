...
const Search = () => {
  ...
  return (
    <MyContext.Provider value={{ details,moviedetails,SetMovieList,ref3 }}>
      <div ref={ref3} className="mt-20 flex flex-row justify-center gap-3 justify-items-center">
        <input
      ...
      <ToastContainer
        ...
        // theme="light"
      />
      <div className="flex flex-wrap flex-row mt-10 gap-4 justify-evenly items-center">
        {movielist && (
          <>
            {moviedetails.map((each) => {
              return (
                ...
                <div key={each?.imdbID} className=" mb-32 flex flex-col gap-2 items-center">
                  ...
                  <p 
                    onClick={handlemovielistdetail} 
                    className=" hover:scale-95 hover:cursor-pointer rounded-lg p-3 mt-5 text-4xl text-sky-950 font-semibold">
                    {each.Title}                    
                  </p>
                </div>
